...
class VRAEEncoder(nn.Module):
    ...
    """
    def __init__(self, number_of_features, args, cnn_kwargs, hidden_layer_depth=2, dropout=0., block = 'LSTM'):
        super(VRAEEncoder, self).__init__()

        self.number_of_features = number_of_features
        self.hidden_size = args.x_fdim2
        self.hidden_layer_depth = hidden_layer_depth
        self.latent_length = args.h_dim
    ...
    def forward(self, x):
        ...
        _, (h_end, c_end) = self.model(x.permute(2,0,1))

        h_end = h_end[-1, :, :]
        return h_end


class VRAELambda(nn.Module):
    ...
    def __init__(self, hidden_size, latent_length):
        super(VRAELambda, self).__init__()
...
class VRAEDecoder(nn.Module):
    ...
    """
    def __init__(self, number_of_features, args, cnn_kwargs, hidden_layer_depth=2, dtype=torch.FloatTensor, block='LSTM'):

        super(VRAEDecoder, self).__init__()

        self.hidden_size = args.x_fdim2
        self.batch_size = args.mb_size
        self.sequence_length = cnn_kwargs[1]
        self.hidden_layer_depth = hidden_layer_depth
        self.latent_length = args.h_dim
        self.output_size = number_of_features
        self.dtype = dtype
        self.device = args.proc
        ...
        self.decoder_inputs = torch.zeros(self.sequence_length, self.batch_size, 1, requires_grad=True, device=args.proc)#.type(self.dtype)
        self.c_0 = torch.zeros(self.hidden_layer_depth, self.batch_size, self.hidden_size, requires_grad=True, device=args.proc)#.type(self.dtype)
    ...
    def myto(self, device):
        self.decoder_inputs = self.decoder_inputs.to(device)
...
class VRAE(nn.Module):
    ...
    def __init__(self, sequence_length, number_of_features, hidden_size=90, hidden_layer_depth=2, latent_length=20,
        ...
        self.encoder = VRAEEncoder(number_of_features = number_of_features,
        ...
        self.lmbd = VRAELambda(hidden_size=hidden_size,
                               latent_length=latent_length)

        self.decoder = VRAEDecoder(sequence_length=sequence_length,
